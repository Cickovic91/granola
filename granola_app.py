...
def check_password():
    if "password_correct" not in st.session_state:
        st.text_input("Unesite pristupnu šifru studija", type="password", on_change=password_entered, key="password")
        return False
...
# --- SIDEBAR NAVIGACIJA ---
page = st.sidebar.radio("Meni", ["Magacin", "Recepti", "Kalkulator Proizvodnje", "Statistika"])
...
if page == "Magacin":
    ...
    with st.expander("Dodaj novu stavku"):
        with st.form("new_item"):
            ...
            if st.form_submit_button("Sačuvaj"):
                jed_cena = cena / grama
                new_row = pd.DataFrame([{"Sirovina": naziv, "Kategorija": kat, "Cena pakovanja": cena, "Kolicina": grama, "Cena po jedinici": jed_cena}])
    ...
    with st.expander("➕ Kreiraj novi recept"):
        ...
        sastojci_opcije = df_magacin[df_magacin['Kategorija'] == 'Sastojak']['Sirovina'].tolist()
        izabrani_sastojci = st.multiselect("Izaberi sastojke iz magacina", sastojci_opcije)
        ...
        if izabrani_sastojci:
            ...
            for s in izabrani_sastojci:
                ...
                cena_po_g = df_magacin[df_magacin['Sirovina'] == s]['Cena po jedinici'].values[0]
                trosak_stavke = grami * cena_po_g
                ukupni_trosak_sirovina += trosak_stavke
                temp_list.append({"Naziv Recepta": recept_ime, "Sastojak": s, "Kolicina_g": grami})
            ...
            st.write(f"🔥 Očekivana težina nakon pečenja (Kalo 0.9): **{pecena_gramaza:.0f}g**")
            st.write(f"💰 Ukupan trošak sirovina: **USD {ukupni_trosak_sirovina:.2f}**") # Koristim USD po instrukcijama
            
            if st.button("Sačuvaj Recept"):
                new_recept_df = pd.concat([df_recepti, pd.DataFrame(temp_list)], ignore_index=True)
    ...
    st.write("Ovde ćemo sutra dodati opciju da izabereš recept i kažeš 'Treba mi 10 tegli', a on ti izbaci šoping listu.")